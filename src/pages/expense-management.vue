      ...
      <div style="width: 100%">
        ...
        <n-form ref="formRef" :model="formValue" :rules="rules" :size="size">
          ...
          <n-form-item label="Vendor Name" path="vendorName">
            <n-input
              v-model:value="formValue.vendorName"
          ...
          <div class="grid-item">
            <template v-if="item.isEditing">
              <div style="display: flex; gap: 8px">
                <n-button
                  @click="item.isNew ? handleRemoveNewItem(index) : handleCancelEdit(item)"
                  size="small"
                  text
                >
        ...
        <div class="expense-info">
          <div class="expense-info-grid">
            <div>
              <div class="paragraph-14-semibold" style="padding-bottom: 0.5rem">Name</div>
              <n-input
                v-model:value="editableExtractedData.name"
                placeholder="Input Name"
                class="paragraph-14-regular input-info"
              />
            </div>
            <div>
              ...
              <n-input
                v-model:value="editableExtractedData.vendorName"
                placeholder="Input Vendor Name"
                class="paragraph-14-regular input-info"
              />
            </div>
            <div>
              ...
              <n-input
                type="date"
                v-model:value="editableExtractedData.date"
                placeholder=""
                class="paragraph-14-regular input-info"
              />
            </div>
            <div>
              ...
              <n-select
                v-model:value="editableExtractedData.categoryId"
                :options="categories"
                filterable
                placeholder="Select Category"
                clearable
...
const handleExport = async () => {
  try {
    loading.value = true
    // Get current sorting and filtering from the table
    const tableState = dataTableRef.value?.getTableState?.() || {}
    const sortBy = tableState.sortBy ? `${tableState.sortBy}:${tableState.sortOrder || 'desc'}` : 'date:desc'
    ...
    if (dataTableRef.value?.searchValue) {
      params.name = dataTableRef.value.searchValue
    }
    
    // Call the export endpoint with the proper URL parameters
    const response = await useApi().get('/expenses/export/csv', params, {
    ...
    notification.success({
      title: 'Success',
      content: 'Expenses exported successfully',
      duration: 3000
    })
  } catch (error) {
    ...
    notification.error({
      title: 'Error',
      content: 'Failed to export expenses',
      duration: 3000
    })
  } finally {
    loading.value = false
...
const fetchCategories = async (page, pageSize, updateLoading) => {
  try {
    ...
    categories.value = response.data.results.map((item) => ({
      label: item.name,
      value: item._id
    }))
    ...
    if (file.size > maxSizeInBytes) {
      ...
      new Compressor(file, {
        ...
        error(err) {
          ...
          notification.error({
            title: 'Error',
            content: 'Failed to compress image. Please try with a different file.',
            duration: 3000
          })
          event.target.value = ''
        }
      ...
      // File is already under 1MB, use it as is
      processFile(file)
    }
  }
}
